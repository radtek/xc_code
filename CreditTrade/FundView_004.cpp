...
void CFundView::SetGridData(CFund& fund)
{
	...
	strCurrentBalance.Format(_T("%0.2f"), fund.current_balance);
	m_gridFund.SetItemTextEx(1, m_gridFund.GetColNum(ColId_CurrentBalance), strCurrentBalance);

	CString strEnableBalance;
	strEnableBalance.Format(_T("%0.2f"), fund.enable_balance);
	m_gridFund.SetItemTextEx(1, m_gridFund.GetColNum(ColId_EnableBalance), strEnableBalance);

	CString strFrozenBalance;
	strFrozenBalance.Format(_T("%0.2f"), fund.frozen_balance);
	m_gridFund.SetItemTextEx(1, m_gridFund.GetColNum(ColId_FrozenBalance), strFrozenBalance);

	CString strUnfrozenBalance;
	strUnfrozenBalance.Format(_T("%0.2f"), fund.unfrozen_balance);
	m_gridFund.SetItemTextEx(1, m_gridFund.GetColNum(ColId_UnfrozenBalance), strUnfrozenBalance);

	CString strRealBuyBalance;
	strRealBuyBalance.Format(_T("%0.2f"), fund.real_buy_balance);
	m_gridFund.SetItemTextEx(1, m_gridFund.GetColNum(ColId_RealBuyBalance), strRealBuyBalance);

	CString strRealSellBalance;
	strRealSellBalance.Format(_T("%0.2f"), fund.real_sell_balance);
...
{
	string s_Fun335101;
	tool::GainQueryFund335101(s_Fun335101,G.SPtr_CData);
	int RefId_HS = tool::SendOrder(G.pCApiFun, Api_Asy, s_Fun335101,G.SPtr_CData);
	if (RefId_HS < 0)
	{
		return false;
	}
	G.SPtr_CData->SetRefId_T2(RefId_HS, RefType_335101_NoFirstQueryFund);
	return true;
}
