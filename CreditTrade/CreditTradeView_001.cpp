...
bool CCreditTradeView::initCreditTradeSettingNeeds()
{
	...
	try {
		...
		obj_CTradeConfig.BuyDefalutGear = pt.get<string>("content.CreditTradeSetting.买默认挡位").c_str();
		obj_CTradeConfig.SellDefalutGear = pt.get<string>("content.CreditTradeSetting.卖默认挡位").c_str();
		obj_CTradeConfig.DefaultAmount = pt.get<int>("content.CreditTradeSetting.默认手数");
		obj_CTradeConfig.DefaultBalance = pt.get<double>("content.CreditTradeSetting.默认金额");
...
void CCreditTradeView::CreateControls()
{
	...
	CSimpleStatic_BEntrustType.Create("委托类型", WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this, IDC_Static_BEntrustType);
	...
	CSimpleStatic_SEntrustType.Create("委托类型", WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this, IDC_Static_SEntrustType);
...
{
	/*股票代码有enchange和killfocus两者处理机制保障只允许存在合法的股票代码或者""，因此平时使用校验是否为空即代表合法，
	这也意味着界面的消息响应里股票代码转市场代码都不可能报错，不过委托起见还是加上，出现即报错，日志，且Assert程序*/
	/*killfocus只用来校验股票代码是否合法(解决enchange没达到特定位数就killcocu的bug)，不合法就进行对应处理*/
