...
int CAlertView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CreateControls();
	m_pCAlertSetting = new CAlertSetting_Credit(this, G.SPtr_CData, G.pCApiFun);
	m_pCAlertSetting->RequireMonitor(G.SPtr_CData->S_StockPath_Credit);
	return 0;
}
	...
	if (nID >= IDC_Check_Refresh && nID <= IDC_Check_AutoScroll)
	{
		...
		case IDC_Check_AutoScroll:
		{
			...
			try {
				...
				pt.put("content.bAutoRoll_Credit", s_tmp);
		...
		{
			G.OnClickedBDirectlyOrder(&m_chcRightMouseOrder, "content.bDirectlyOrder_Credit");
			break;
		}
...
bool CAlertView::GainBuy335002(string &s_str, const CString stock_code, const double &entrust_price, UInt_64 &entrust_amount)
{
	...
	string entrust_type;
	switch (tool::find_enumEntrustType(string(obj_CTradeConfig.BEntrustType_Alert)))
	{
...
void CAlertView::dealSendBuyEntrust_Directly(const CString &stock_code, const CString &MarketCode)
{
	...
	if (G.gainBEntrustAmount_Directly(GainbFinancingBuy(obj_CTradeConfig.BEntrustType_Alert), entrust_price, entrust_amount,
		obj_CTradeConfig.bCalculateByBalance, obj_CTradeConfig.DefaultBalance, obj_CTradeConfig.DefaultAmount) == false)
	{
	...
	string s_Fun335002;
	if (GainBuy335002(s_Fun335002, stock_code, entrust_price, entrust_amount) == false)
	{
		return;
	}
...
bool CAlertView::GainbFinancingBuy(CString BEntrustType)
{
...
void CAlertView::InitGrid()
{
	...
	//m_gridAlert.SetFont(CFont::FromHandle(SingleFontSet.GetDefaultFont(DEFAULT_FONT_14N)));
...
void CAlertView::GetVirtualCellInfo(GV_DISPINFO & dispinfo, const CAlertNeed& alertNeed)
{
	...
	switch (nID)
	{
	case ColId_whetherBiaoDi:
	{
		dispinfo.item.strText = alertNeed.s_bBiaoDiQuan;
		break;
	}
	...
	case COLID_stockCode:
	{
		...
		/*if (G.highlightedSecCode == alertNeed.SecCode)
		{
			...
			m_gridAlert.SetSelectedRangeEx(row, 0, row, nColCount - 1, TRUE);
		}*/
		break;
	}
	case ColId_RiseRate:
