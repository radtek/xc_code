...
#include "stdafx.h"
#include "AlertSetting_Credit.h"
#include "afxdialogex.h"


// CAlertSetting 对话框

IMPLEMENT_DYNAMIC(CAlertSetting_Credit, CDialogEx)

CAlertSetting_Credit::CAlertSetting_Credit(CWnd* pParent /*=NULL*/, PtrData SPtr_CDataTmp,CApiFun *pCApiFunTmp)
...
CAlertSetting_Credit::~CAlertSetting_Credit()
{
}

void CAlertSetting_Credit::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CAlertSetting_Credit, CDialogEx)
	ON_BN_CLICKED(IDOK, &CAlertSetting_Credit::OnBnClickedOk)
	ON_EN_CHANGE(IDC_IncreaseRatio, &CAlertSetting_Credit::OnEnChangeIncreaseratio)
	ON_EN_CHANGE(IDC_IncreaseRatio_N_Rise, &CAlertSetting_Credit::OnEnChangeIncreaseRatioNRise)
	ON_EN_CHANGE(IDC_BeginRatio_Low_Rise, &CAlertSetting_Credit::OnEnChangeBeginRatioLowRise)
	ON_EN_CHANGE(IDC_EndRatio_Low_Rise, &CAlertSetting_Credit::OnEnChangeEndRatioLowRise)
	ON_EN_CHANGE(IDC_Minute_N_Rise, &CAlertSetting_Credit::OnEnChangeMinuteNRise)
	ON_EN_CHANGE(IDC_Minute_Low_Rise, &CAlertSetting_Credit::OnEnChangeMinuteLowRise)
END_MESSAGE_MAP()

// CAlertSetting_Credit 消息处理程序

BOOL CAlertSetting_Credit::OnInitDialog()
{
...
bool CAlertSetting_Credit::initAlertSettingNeed()
{
	return CAlertSetting::initAlertSettingNeed("content.AlertSetting_Credit.");
}

void CAlertSetting_Credit::initAlertSettingLayout()
{	
...
bool CAlertSetting_Credit::VerifyBeforeSave()
{
	CString s_str;
...
bool CAlertSetting_Credit::SaveAlertSettingNeed()
{
	/*涨幅*/
	string Node_Father = "content.AlertSetting_Credit.";
...
void CAlertSetting_Credit::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	if (VerifyBeforeSave() == true)
	{
		...
		{
			LOG(ERROR) << "ClearMAlert_All fail";
			ExceptionInterrupt;
			return;
		}
		/*删除预警池现有数据*/
		::SendMessage(SPtr_CData->hwnd_TradeDlg, WM_RedrawAlert, 0, 0);
		RequireMonitor(SPtr_CData->S_StockPath_Credit);
	}
	else
	{
		return;
...
void CAlertSetting_Credit::OnEnChangeIncreaseratio()
{
...
void CAlertSetting_Credit::OnCancel()
{
...
void CAlertSetting_Credit::OnIdclose()
{
	// TODO: Add your command handler code here
}


void CAlertSetting_Credit::OnEnChangeIncreaseRatioNRise()
{
...
void CAlertSetting_Credit::OnEnChangeBeginRatioLowRise()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 __super::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CAlertSetting_Credit::OnEnChangeEndRatioLowRise()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 __super::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CAlertSetting_Credit::OnEnChangeMinuteNRise()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 __super::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CAlertSetting_Credit::OnEnChangeMinuteLowRise()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 __super::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
