...
int COrderView::m_nColID = -1;

BOOL COrderView::m_bAscending = FALSE;
...
CString GetEntrustType(enumEntrustType type)
{
	CString strEntrustType;
	switch (type)
	{
...
void COrderView::OnGridOrderLMouseDown(NMHDR *pNmhdr, LRESULT *pResult)
{
	...
	if (!((CCreditTradeView*)GetViewByID(ID_viw_creditTrade))->VerifyEntrustTyep(SecCode, true))
	{
		return;
	}
	((CCreditTradeView*)GetViewByID(ID_viw_creditTrade))->RedrawTradeModule(true, SecCode);
}
...
void COrderView::InitGrid()
{
	...
	m_gridOrder.SetVirtualCompare(CompareVirtual);
...
{
	string s_Fun335004;
	if (GainRevoke335004(s_Fun335004, entrust_no, exchange_type) == false)
	{
		LOG(ERROR) << "GainRevoke335004 fail";
		ExceptionInterrupt;
		return false;
	}
	if (tool::SendOrder(G.pCApiFun, Api_Asy, s_Fun335004, G.SPtr_CData)< 0)
	{
		return false;
	}
...
bool COrderView::GainRevoke335004(string &s_str, const UInt_64 entrust_no, const string &exchange_type)
{
	//委托撤单
	Json::Value obj_Value;
	obj_Value["function_id"] = "335004";
...
void COrderView::GetVirtualCellInfo(GV_DISPINFO & dispinfo, const COrder &order)
{
	...
	case ColId_EntrustType:
	{
		dispinfo.item.strText = GetEntrustType(order.entrust_type);
		break;
	}
	...
	default:
		...
		// 	case COLID_BSFLAG:
		// 	{
		// 		dispinfo.item.crFgClr = GetDirectionColor(order.Direction);
		// 		dispinfo.item.strText = GetDirection2(order.Direction);
		// 		dispinfo.item.fTick = order.getInstrument() == nullptr ? 1 : order.getInstrument()->PriceTick.getValue();
		// 		dispinfo.item.nPrecision = order.getInstrument() == nullptr ? 2 : order.getInstrument()->PricePrecision;
		// 		break;
		// 	}
...
bool COrderView::CompareVirtual(int nCol, BOOL bAscending, LPARAM lParam)
{
	COrderView* pOrder = (COrderView *)lParam;
	if (pOrder == nullptr)
	{
		return false;
	}
	COrderView::m_bAscending = bAscending;
	COrderView::m_nColID = nCol;
	std::stable_sort(G.VOrder_Redraw.begin(), G.VOrder_Redraw.end(), CompareOrderData);
	return true;
}

BOOL COrderView::CompareOrderData(PtrOrder order1, PtrOrder order2)
{
...
void COrderView::GetVirtualCellText(CString &strText, PtrOrder pOrder, const int nColID)
{
	if (nullptr == pOrder)
	{
		return;
	...
	case ColId_stockName:
	{
		strText = pOrder->stock_name;
		break;
	}
	case COLID_stockCode:
	{
		strText = pOrder->stock_code;
		break;
	}
	case COLID_direction:
	{
		strText = GetDirection(pOrder->entrust_bs);
		break;
	}
	case ColId_EntrustPrice:
	{
		CString str;
		str.Format(_T("%0.2f"), pOrder->entrust_price);
		strText = str;
		break;
	}
	case ColId_EntrstAmount:
	{
		CString str;
		str.Format(_T("%u"), pOrder->entrust_amount);
		strText = str;
		break;
	}
	case ColId_BusinessAmount:
	{
		CString str;
		str.Format(_T("%u"), pOrder->business_amount);
		strText = str;
		break;
	}
	case COLID_entrustStatus:
	{
		strText = GetStatus(pOrder->entrust_status);
		break;
	}
	case ColId_EntrustNo:
	{
		CString str;
		str.Format(_T("%u"), pOrder->entrust_no);
		strText = str;
		break;
	}
	case COLID_reportTime:
	{
		strText = pOrder->report_time;
		break;
	}
