...
namespace tinyxml2
{
	...
	char* XMLNode::ParseDeep(char* p, StrPair* parentEnd, int* curLineNumPtr)
	{
		...
		while (p && *p) {
			...
			if (!p) {
				DeleteNode(node);
				if (!_document->Error()) {
					_document->SetError(XML_ERROR_PARSING, nullptr, nullptr, initialLineNum);
				}
				break;
			}
			...
			if (decl) {
				...
				bool wellLocated = (ToDocument() != nullptr);
				if (wellLocated) {
				...
				if (!wellLocated) {
					_document->SetError(XML_ERROR_PARSING_DECLARATION, decl->Value(), nullptr, initialLineNum);
					DeleteNode(node);
					break;
				}
			}
			...
			if (ele) {
				...
				if (mismatch) {
					_document->SetError(XML_ERROR_MISMATCHED_ELEMENT, ele->Name(), nullptr, initialLineNum);
					DeleteNode(node);
					break;
				}
			}
	...
	char* XMLText::ParseDeep(char* p, StrPair*, int* curLineNumPtr)
	{
		...
		if (this->CData()) {
			...
			if (!p) {
				_document->SetError(XML_ERROR_PARSING_CDATA, start, nullptr, _parseLineNum);
			}
			return p;
		}
		else {
			...
			if (!p) {
				_document->SetError(XML_ERROR_PARSING_TEXT, start, nullptr, _parseLineNum);
			}
		}
		return nullptr;
	}
	...
	char* XMLComment::ParseDeep(char* p, StrPair*, int* curLineNumPtr)
	{
		...
		if (p == nullptr) {
			_document->SetError(XML_ERROR_PARSING_COMMENT, start, nullptr, _parseLineNum);
		}
		return p;
	}
	...
	char* XMLDeclaration::ParseDeep(char* p, StrPair*, int* curLineNumPtr)
	{
		...
		p = _value.ParseText(p, "?>", StrPair::NEEDS_NEWLINE_NORMALIZATION, curLineNumPtr);
		if (p == nullptr) {
			_document->SetError(XML_ERROR_PARSING_DECLARATION, start, nullptr, _parseLineNum);
		}
		return p;
	}
	...
	char* XMLUnknown::ParseDeep(char* p, StrPair*, int* curLineNumPtr)
	{
		...
		p = _value.ParseText(p, ">", StrPair::NEEDS_NEWLINE_NORMALIZATION, curLineNumPtr);
		if (!p) {
			_document->SetError(XML_ERROR_PARSING_UNKNOWN, start, nullptr, _parseLineNum);
		}
		return p;
	}
	...
	XMLError XMLDocument::LoadFile(const char* filename)
	{
		...
		if (!fp) {
			SetError(XML_ERROR_FILE_NOT_FOUND, filename, nullptr, 0);
			return _errorID;
		}
		LoadFile(fp);
		fclose(fp);
	...
	XMLError XMLDocument::LoadFile(FILE* fp)
	{
		...
		if (fgetc(fp) == EOF && ferror(fp) != 0) {
			SetError(XML_ERROR_FILE_READ_ERROR, nullptr, nullptr, 0);
			return _errorID;
		}
		...
		if (filelength == 0) {
			SetError(XML_ERROR_EMPTY_DOCUMENT, nullptr, nullptr, 0);
			return _errorID;
		}
	...
	XMLError XMLDocument::SaveFile(const char* filename, bool compact)
	{
		...
		if (!fp) {
			SetError(XML_ERROR_FILE_COULD_NOT_BE_OPENED, filename, nullptr, 0);
			return _errorID;
		}
		SaveFile(fp, compact);
		fclose(fp);
