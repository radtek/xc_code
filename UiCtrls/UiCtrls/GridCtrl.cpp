...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_crGridLineColour    = RGB(66,66,66);   
    m_clrHighlightSetFocus = RGB(61, 91, 153);
    m_clrHighlightKillFocus = RGB(41, 62, 103);
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    //发送OnKeyDown消息，此消息比较特殊，第一个参数表示nChar，接收方也做类似处理
    SendMessageToParent(nChar, 0, NM_KEYDOWN);
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (GetVirtualMode())
	{
		...
		dispinfo.item.row     = pgvItem->row;
		dispinfo.item.col     = pgvItem->col;
		dispinfo.item.strText  = pgvItem->strText;
		CString strCurrentText = GetItemText(pgvItem->row, pgvItem->col);
		CWnd *pOwner = GetOwner();
		if (pOwner && IsWindow(pOwner->m_hWnd) && pgvItem->strText != strCurrentText)
			pOwner->SendMessage(WM_NOTIFY, pgvDispInfo->hdr.idFrom, (LPARAM)&dispinfo);
    ...
    case SB_THUMBTRACK:
        {
            ...
            {
                rect.top = GetFixedRowHeight();
				m_bEnableSelectHighlight = FALSE;
// 				if (m_bEnableScrollSelectRow)
// 				{
// 					CCellID idTopLeft = GetTopleftNonFixedCell();
// 					int nColCount = GetColumnCount();
// 					SetSelectedRangeEx(idTopLeft.row, 0, idTopLeft.row, nColCount - 1, TRUE);
// 				}			
                InvalidateRect(rect);
            }
        }
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    for (row = 0; row < m_nFixedRows; row++)
    {
        ...
        for (col = 0; col < m_nFixedCols; col++)
        {
            ...
            if (pCell)
			{
                ...
                pCell->Draw2(pDC, row, col, rect, FALSE);
			}
        }
    }
    ...
    for (row = minVisibleRow; row <= maxVisibleRow; row++)
    {
        ...
        for (col = minVisibleCol; col <= maxVisibleCol; col++)
        {
			...
			{
				pCell->SetCoords(row,col);
				// modify by yangyong 20170330
				if (!m_bTextBackCrossing)
					pCell->Draw2(pDC, row, col, rect, FALSE);
				else
					pCell->Draw2(pDC, row, col, rect, TRUE);
			}
        }
    }
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
    ...
    if (pDC)
    {
        ...
        if (nRow < m_nFixedRows || nCol < m_nFixedCols)
        {
            ...
            if (pCell)
                bResult = pCell->Draw2(pDC, nRow, nCol, rect, TRUE);
        }
        else
        {
            CGridCellBase* pCell = GetCell(nRow, nCol);
	...
	BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	...
	//CWaitCursor wait; // Fuck u,Thomas Haase 

					  //只调用SetSelectedRange时候，此行并未真正选中，故添加此代码 yaoshulong 2018/02/23
	if (::IsWindow(this->GetSafeHwnd()) && nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0)
	{
		SetFocus();
		SetFocusCell(nMinRow, 0);
	}
	...
	int maxVisibleRow = VisCellRange.GetMaxRow(),
		maxVisibleCol = VisCellRange.GetMaxCol();
	if (nMinRow > maxVisibleRow + idTopLeft.row)
	{
		int nYScroll = 1;
		for (int i = maxVisibleRow + idTopLeft.row + 1; i <= nMinRow; i++)
		{
			nYScroll += GetRowHeight(i);
		}
		SetScrollPos32(SB_VERT, scrollPos + nYScroll);
		Invalidate(FALSE);
	}
	else if (nMinRow < idTopLeft.row && nMinRow > 0)
	{
		int nYScroll = 0;
		for (int i = nMinRow; i < idTopLeft.row; i++)
		{
			nYScroll += GetRowHeight(i);
		}
		SetScrollPos32(SB_VERT, scrollPos - nYScroll);
...
void CGridCtrl::MySetSelectedRangeEx(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
	BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	if (!m_bEnableSelection || !IsWindow(GetSafeHwnd()))
		return;
	...
	int maxVisibleRow = VisCellRange.GetMaxRow(),
		maxVisibleCol = VisCellRange.GetMaxCol();
	int nGapRow = maxVisibleRow - idTopLeft.row;
	if (nMinRow > maxVisibleRow)
	{
		int nStart = 0;
		if (m_bEnableSelectOnTopMost)
		{
			nStart = maxVisibleRow - nGapRow + 1;
		}
		else
		{
			nStart = maxVisibleRow + 1;
		}

		int nYScroll = 1;
		for (int i = nStart; i <= nMinRow; i++)
		{
			nYScroll += GetRowHeight(i);
		}
		SetScrollPos32(SB_VERT, scrollPos + nYScroll);
	...
	else
	{
		if (m_bEnableSelectOnTopMost)
		{
			int nRowToTop = nMinRow - idTopLeft.row - 1;
			int nYScroll = 1;
			for (int i = 1; i <= nRowToTop; i++)
			{
				nYScroll += GetRowHeight(i);
			}
			SetScrollPos32(SB_VERT, scrollPos + nYScroll);
                       ...
                       CPoint /* point */)
{
    ...
    if (m_CurCol>=0)
	{
		...
		else
		{
			...
			OnChangeColPos(m_CurCol, m_LastDragOverCell.col);
...
CCellID CGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/)
{
    ...
    int nVertScroll = GetScrollPos32(SB_VERT)/*GetScrollPos(SB_VERT)*/, 
        nHorzScroll = GetScrollPos32(SB_HORZ)/* GetScrollPos(SB_HORZ)*/;
...
LRESULT CGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */)
{
	...
	int nSize = m_arRowHeights.GetSize();
	if (nSize > 0)
	{
		m_arRowHeights[0] = m_cellDefault.GetHeight();
	}
                            ...
                            int nColumn  /* = -1 */)
{
	...
	// black Inc the m_nCols is a BUG!  Inc the count by calling SetColumnCount() will cause memory leaks
	// must reset Column Order
    m_nCols++;
	//SetColumnCount(m_nCols+1);  // cause memory leaks
...
BOOL CGridCtrl::SetCellComboBox(int nRow, int nCol, const CStringArray& options, CString strSel)
{
        CRuntimeClass* pRuntimeClass = RUNTIME_CLASS(CGridCellCombo);
        if (GetVirtualMode())
            return FALSE;
        ...
        SetItemText(nRow,nCol, strSel);

        CGridCellCombo *pCell = (CGridCellCombo*)GetCell(nRow,nCol);
...
BOOL CGridCtrl::SetCellCheckBox(int nRow, int nCol)
{
    CRuntimeClass* pRuntimeClass = RUNTIME_CLASS(CGridCellCheck);
    if (GetVirtualMode())
        return FALSE;
...
BOOL CGridCtrl::SetCellHotkeyEdit(int nRow, int nCol)
{
    CRuntimeClass* pRuntimeClass = RUNTIME_CLASS(CGridCellHotkeyEdit);
    if (GetVirtualMode())
        return FALSE;
...
BOOL CGridCtrl::SetCellTime(int nRow, int nCol)
{
    CRuntimeClass* pRuntimeClass = RUNTIME_CLASS(CGridCellTime);
    if (GetVirtualMode())
        return FALSE;
...
BOOL CGridCtrl::SetCellButtonEdit(int nRow, int nCol)
{
	CRuntimeClass* pRuntimeClass = RUNTIME_CLASS(CGridCellButtonEdit);
	if (GetVirtualMode())
		return FALSE;
...
BOOL CGridCtrl::SetCellNumberEdit(int nRow, int nCol)
{
    CRuntimeClass* pRuntimeClass = RUNTIME_CLASS(CGridCellNumberEdit);
    if (GetVirtualMode())
        return FALSE;
...
BOOL CGridCtrl::SetCellDoubleEdit(int nRow, int nCol)
{
    CRuntimeClass* pRuntimeClass = RUNTIME_CLASS(CGridCellDoubleEdit);
    if (GetVirtualMode())
        return FALSE;
...
BOOL CGridCtrl::GetCellCheck(int nRow, int nCol)
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    return pCell->GetCheck();
}

void CGridCtrl::SetCellCheck(int nRow, int nCol, BOOL bChecked/* = FALSE*/)
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    ...
    while (lo <= hi)
    {
        ...
        if (lo <= hi)
        {
            ...
            if (pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)GetCell(hi, nCol), data) != 0)
            {
				for (int col = 0; col < GetColumnCount(); col++)
                {
					// modify by yangyong 20170508
					CGridCellBase *pCell = GetCell2(lo, col);
                    SetCell(lo, col, GetCell2(hi, col));
...
BOOL CGridCtrl::SetItemEditable(int nRow, int nCol, BOOL bEditable)
{
    if (GetVirtualMode())
        return FALSE;
    ...
    pCell->SetEditable(bEditable);
    return TRUE;
}

void CGridCtrl::SetItemBtnEditSetting(int nRow, int nCol, double fTick, int nPrecision, BtnEditDataType dateType /*= TYPE_DOUBLE*/)
{
	if (GetVirtualMode())
		return ;
	...
	pCell->SetTick(fTick);
	pCell->SetPrecision(nPrecision);
	pCell->SetDateType(dateType);
}
...
BOOL CGridCtrl::GetItemEditable(int nRow, int nCol) const
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
...
void CGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    ...
    for (nCol = 0; nCol < nNumColumns; nCol++)
    {
        ...
        if( GetColumnWidth( nCol) > 0 )
        {
            ...
            m_arColWidths[m_arColOrder[nCol]] = 0;
            for (nRow = nStartRow; nRow <= nEndRow; nRow++)
            {
                if( GetRowHeight( nRow) > 0 )
                {
                    ...
                    if (pCell)
                        size = pCell->GetCellExtent(pDC);
                    if (size.cx >(int) m_arColWidths[m_arColOrder[nCol]])
                        m_arColWidths[m_arColOrder[nCol]] = size.cx;
...
#endif
            SendMessageToParent(idCurrentCell.row, idCurrentCell.col, GVN_CELLMOUSEHOVER);
        }
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_SIZING_COL)
    {
        ...
        {   
			...
			OnChangeColWidth(m_LeftClickDownCell.col, nColumnWidth);
            ResetScrollBars();
            Invalidate();
        }
    }
    ...
    else
    {
        ...
        CCellID cell  = GetCellFromPt(point);
        if (cell.row == m_idCurrentCell.row)
        {
            SendMessageToParent(cell.row, cell.col, GVN_SELCHANGED);

            CGridCellBase* pCell = GetCell(cell.row, cell.col);
            if (pCell)
                pCell->OnClick( GetPointClicked( cell.row, cell.col, point) );
            //SendMessageToParent(cell.row, cell.col, NM_CLICK);
        }
    }
...
void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
    ...
    {
        SendMessageToParent(-1, -1, GVN_SELCHANGED);//add by yaoshulong 2017.12.27
        SendMessageToParent(-1, -1, NM_RCLICK);
    }
    else
    {
		if (0 == SendMessageToParent(FocusCell.row, FocusCell.col, NM_RCLICK))
		{
			SetFocusCell(-1, -1);
			SetFocusCell(__max(FocusCell.row, m_nFixedRows),
				__max(FocusCell.col, m_nFixedCols));
			SelectRowsEx(GetCellFromPt(point), FALSE);
			// tell the cell about it
...
//         if (pCell)
//             pCell->OnRClick( GetPointClicked( FocusCell.row, FocusCell.col, point) );
//         SendMessageToParent(FocusCell.row, FocusCell.col, GVN_SELCHANGED);//add by yaoshulong 2017.12.27
//         SendMessageToParent(FocusCell.row, FocusCell.col, NM_RCLICK);
//     }
}
#endif
