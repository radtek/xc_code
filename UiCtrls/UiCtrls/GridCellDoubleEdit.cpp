...
IMPLEMENT_DYNCREATE(CGridCellDoubleEdit, CGridCell)

CGridCellDoubleEdit::CGridCellDoubleEdit() : CGridCell()
{
}

BOOL CGridCellDoubleEdit::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    if ( m_bEditing )
    {      
        if (m_pEditWnd)
    ...
    {   
        ...
        m_pEditWnd = new CInPlaceIntEdit(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar, DOUBLE_INPUT);
    }
    return TRUE;
}
// Cancel the editing.
void CGridCellDoubleEdit::EndEdit()
{
    if (m_pEditWnd)
