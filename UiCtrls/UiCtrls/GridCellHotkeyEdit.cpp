...
IMPLEMENT_DYNCREATE(CGridCellHotkeyEdit, CGridCell)

CGridCellHotkeyEdit::CGridCellHotkeyEdit() : CGridCell()
{
}

BOOL CGridCellHotkeyEdit::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    if ( m_bEditing )
    {      
        if (m_pEditWnd)
    ...
    {   
        ...
        m_pEditWnd = new CInPlaceHotkeyEdit(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar);
    }
    return TRUE;
}
// Cancel the editing.
void CGridCellHotkeyEdit::EndEdit()
{
    if (m_pEditWnd)
        ((CInPlaceHotkeyEdit*)m_pEditWnd)->EndEdit();
}
