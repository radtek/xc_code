...
void CTTComboBox::CreateTooltipWnd()
{
	...
	if (!::IsWindow(m_tipWnd.m_hWnd))
	{
		...
		WNDPROC oldTipWndProc = (WNDPROC)::SetWindowLong(m_tipWnd.m_hWnd, 
			GWL_WNDPROC, (LONG)HookTipWndProc);
...
void CTTComboBox::InstallHookForListboxAndEditbox()
{
	...
	if (m_cbi.hwndList)
	{
		...
		WNDPROC oldListWndProc = (WNDPROC)::SetWindowLong(m_cbi.hwndList, GWL_WNDPROC, (LONG)HookListboxWndProc);
		// Add the old hook(i.e, hook associated with window) to the map
	...
	if (m_cbi.hwndItem)
	{
		...
		WNDPROC oldEditWndProc = (WNDPROC)::SetWindowLong(m_cbi.hwndItem, GWL_WNDPROC, (LONG)HookEditboxWndProc);
		// Add the old hook(i.e, hook associated with window) to the map
