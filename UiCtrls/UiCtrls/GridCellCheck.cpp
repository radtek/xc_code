...
BOOL CGridCellCheck::Draw2(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
...
// 	    (m_bChecked)? DFCS_BUTTONCHECK | DFCS_CHECKED : DFCS_BUTTONCHECK);
	...
	HBITMAP hBitmap = (HBITMAP)LoadImage(AfxGetInstanceHandle(), m_bChecked ? sPath + "\\Bmp\\checkGrid.bmp" : sPath + "\\Bmp\\uncheckGrid.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	...
	CRect rectGet = GetCheckPlacement();

	pDC->BitBlt(rectGet.left, rectGet.top, 17, 17, &dcMem, 0, 0, SRCCOPY);
...
void CGridCellCheck::SetCheck(BOOL bChecked /*=TRUE*/)
{
	m_bChecked = bChecked;
