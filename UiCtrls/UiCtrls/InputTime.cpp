...
IMPLEMENT_DYNAMIC(CInputTime, CWnd)

	CInputTime::CInputTime()
{
	RegisterWindowClass();
...
BOOL CInputTime::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{

	return CWnd::Create(INPUTTIME_CLASSNAME, "", dwStyle, rect, pParentWnd, nID);
}

void CInputTime::SetBkColor(COLORREF clr)
{
	m_clrBk = clr;
	Invalidate(FALSE);
}

void CInputTime::SetTextColor(COLORREF clrNormal, COLORREF clrDisable)
{
...
void CInputTime::SetFont(HFONT font)
{
	m_Font = font;
	Invalidate(FALSE);
}

void CInputTime::EnableWindow(BOOL bEnable /*= TRUE*/)
{
...
void CInputTime::SetChecked(BOOL bChecked /*= FALSE*/)
{
	if (!m_bEnable)
	{
		return;
...
BOOL CInputTime::GetChecked()
{
	return m_bChecked;
}

void CInputTime::SetBoxShow(BOOL bShow /*= TRUE*/)
{
	if (!m_bEnable)
	{
		return;
...
BOOL CInputTime::GetBoxShow()
{
	return m_bBoxShow;
}

void CInputTime::SetMode(UINT mode /*= EDITABLE_WHILE_CHECKED*/)
{
	if (!m_bEnable)
	{
		return;
...
UINT CInputTime::GetMode()
{
	return m_mode;
}

void CInputTime::GetTime(CTime &tm)
{
	SYSTEMTIME st;
	GetLocalTime(&st);
...
void CInputTime::SetTime(CTime tm)
{
	m_nHour = tm.GetHour();
	m_nMin = tm.GetMinute();
	m_nSec = tm.GetSecond();
...
void CInputTime::SetTime(CString strTime)
{
...
BOOL CInputTime::RegisterWindowClass()
{
	WNDCLASS wndcls;
	HINSTANCE hInst = AfxGetInstanceHandle();

	if (!(::GetClassInfo(hInst, INPUTTIME_CLASSNAME, &wndcls)))
	{
		...
		wndcls.lpszClassName    = INPUTTIME_CLASSNAME;
...
void CInputTime::DrawUnchecked(CDC &dc, CRect rt, COLORREF clrA, COLORREF clrB, COLORREF clrC)
{
	CPen pen1;
...
void CInputTime::DrawChecked(CDC &dc, CRect rt, COLORREF clrA, COLORREF clrB)
{
	CPen pen1;
	pen1.CreatePen(PS_SOLID, 1, clrA);
	dc.SelectObject(pen1);
...
BEGIN_MESSAGE_MAP(CInputTime, CWnd)
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	ON_WM_CREATE()
...
void CInputTime::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (!m_bEnable)
	{
		return;
...
void CInputTime::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if (!m_bEnable)
	{
		return;
...
int CInputTime::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
HBRUSH CInputTime::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CInputTime::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (!m_bEnable)
	{
		return;
...
void CInputTime::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (!m_bEnable)
	{
		return;
...
BOOL CInputTime::PreTranslateMessage(MSG* pMsg)
{
	if (!m_bEnable)
	{
...
BOOL CInputTime::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}


void CInputTime::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (!m_bEnable)
	{
		return;
