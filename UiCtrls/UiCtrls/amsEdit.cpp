...
#pragma warning (disable:4407)  // disables: 在指向成员表示形式的不同指针之间进行转换，编译器可能生成不正确的代码
...
LONG CAMSEdit::OnSetText(UINT, LONG lParam)
{
	...
	if (pDialog->IsKindOf(RUNTIME_CLASS(CComboBox)))
	{
		strText = GetText();
	}
	else
	{
		strText = GetValidText();		
	}
...
CString CAMSEdit::MaskedBehavior::_GetValidText() const
{
	...
	for (int iPos = 0, iMaskPos = 0, nLen = strText.GetLength(); iPos < nLen; iPos++, iMaskPos++)
	{
		...
		int iSymbol = 0, nSymbolCount = m_arraySymbols.GetSize();
...
void CAMSEdit::MaskedBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	...
	int nSymbolCount = m_arraySymbols.GetSize();
	for (int iSymbol = 0; iSymbol < nSymbolCount; iSymbol++)
	{
		const Symbol& symbol = m_arraySymbols[iSymbol];
		if (cMask == symbol)
		{
