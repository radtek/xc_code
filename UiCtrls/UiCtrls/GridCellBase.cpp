...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	{
		// modify by yangyong 20170328
		if (pGrid->m_bTextBackCrossing)
		{
			if (nRow % 2 == 0)
			{
			...
			{
				TextBkClr = pDefaultCell->GetBackClr2();
			}
		}
		else
		{
    ...
    if ( IsFocused() || IsDropHighlighted() )
    {
        ...
        {
            TextBkClr = pGrid->GetGridHighlightColor();
    ...
    {
        rect.right++; rect.bottom++;    // FillRect doesn't draw RHS or bottom
        pDC->FillSolidRect(rect, pGrid->GetGridHighlightColor());
...
BOOL CGridCellBase::Draw2(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    // Note - all through this function we totally brutalise 'rect'. Do not
    // depend on it's value being that which was passed in.
    ...
    if (GetBackClr() == CLR_DEFAULT)
	{
		...
		if (pGrid->m_bTextBackCrossing)
		{
			if (nRow % 2 == 0)
			{
				hBkBrush = pDefaultCell->GetBackBrush1();
			}
			else
			{
				hBkBrush = pDefaultCell->GetBackBrush2();
			}
		}
		else
		{
    ...
    {
        bEraseBkgnd = TRUE;
		hBkBrush = CreateSolidBrush(GetBackClr());
        bMustReleaseBrush = TRUE;
    }
    ...
    if ( IsFocused() || IsDropHighlighted() )
    {
        ...
        if(GetState() & GVIS_SELECTED)
        {
            if (bMustReleaseBrush)
            {
                bMustReleaseBrush = FALSE;
                DeleteObject(hBkBrush);
                hBkBrush = NULL;
            }
            hBkBrush = pGrid->GetGridHighlightHbrush();		
         //   TextClr = ::GetSysColor(COLOR_HIGHLIGHTTEXT);
            bEraseBkgnd = TRUE;
        }
		...
		if (bEraseBkgnd)
		{
			::FillRect(pDC->m_hDC, &rect, hBkBrush);
		}
...
//             rect.right--;
            CPen pen;
            pen.CreatePen(PS_SOLID, 1, /*pGrid->GetGridLineColor()*/RGB(95,95,95));

            CPen* pOldPen = (CPen*) pDC->SelectObject(&pen);
            
           /* pDC->MoveTo(rect.right, rect.top);
            pDC->LineTo(0 == rect.left ? rect.left : rect.left - 1, rect.top);
            pDC->LineTo(0 == rect.left ? rect.left : rect.left - 1, rect.bottom);*/
			if (pGrid->GetColumnCount() != nCol + 1)
			{
        ...
        if (size.cx + rect.left < rect.right)
        {
            ...
            if (pGrid->GetSortAscending())
            {
              ...
              /*  pDC->MoveTo( nTriangleLeft + 1, nTriangleBase + size.cy + 1);
                ...
                pDC->LineTo( nTriangleLeft + size.cx, nTriangleBase + size.cy );
                pDC->LineTo( nTriangleLeft, nTriangleBase + size.cy );*/
				int nLeft = nTriangleLeft+ (size.cx / 2);
				int nTop = rect.top + (rect.Height() - size.cy) / 2;
				for (int i = 0; i < 6; i++)
				{
					nTop += 1;
					pDC->MoveTo(nLeft - i, nTop);
					pDC->LineTo(nLeft + i + 1, nTop);
				}
                pDC->SelectObject(pOldPen);
            }
            else
            {
                ...
                CPen *pOldPen = (CPen*) pDC->SelectObject(&penLight);
				int nLeft = nTriangleLeft+ (size.cx / 2);
				int nTop = rect.top + (rect.Height() - size.cy) / 2  + size.cy / 2 + 3;
				for (int i = 0; i < 6; i++)
				{
					nTop -= 1;
					pDC->MoveTo(nLeft - i, nTop);
