...
IMPLEMENT_DYNCREATE(CGridCellTime, CGridCell)

CGridCellTime::CGridCellTime() : CGridCell()
{
}

BOOL CGridCellTime::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    if ( m_bEditing )
    {      
        if (m_pEditWnd)
    ...
    {   
        ...
        m_pEditWnd = new CInPlaceTime(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar);
    }
    return TRUE;
}
// Cancel the editing.
void CGridCellTime::EndEdit()
{
    if (m_pEditWnd)
        ((CInPlaceTime*)m_pEditWnd)->EndEdit();
}
