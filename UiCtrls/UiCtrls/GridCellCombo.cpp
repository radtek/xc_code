						   ...
						   UINT nFirstChar)
{
    ...
    SetFontEx(pParent->GetFont());
    SetBkColor(RGB(18, 20, 22));
...
BEGIN_MESSAGE_MAP(CInPlaceList, CSimpleComboBox)
...
{
	CSimpleComboBox::OnKillFocus(pNewWnd);
...
void CInPlaceList::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	CSimpleComboBox::OnKeyDown(nChar, nRepCnt, nFlags);
}
...
void CInPlaceList::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	CSimpleComboBox::OnKeyUp(nChar, nRepCnt, nFlags);
}
...
BOOL CGridCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    ...
    DWORD dwStyle = GetStyle() | CBS_DROPDOWNLIST | CBS_HASSTRINGS;
    m_pEditWnd = new CInPlaceList(GetGrid(), rect, dwStyle, nID, nRow, nCol, 
...
BOOL CGridCellCombo::Draw2(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
#ifdef _WIN32_WCE
