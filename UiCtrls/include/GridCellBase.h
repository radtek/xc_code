...
#define CellStyle_CheckBox        1
#define CellStyle_ComboBox      2
#define CellStyle_ButtonEdit       3
...
public:
    ...
    virtual void SetEditable(BOOL /*bEditable*/)              = 0 ;//add by yaoshulong 11.29.2017
	...
	virtual void SetTick(double fTick) = 0;
	virtual double GetTick() const = 0;
	virtual void SetPrecision(int nPrecision) = 0;
	virtual int GetPrecision() const = 0;
	virtual void SetDateType(int nDateType) = 0;
	virtual int GetDateType() const = 0;


    virtual LPCTSTR    GetText()       const                = 0 ;
    virtual BOOL    GetEditable()       const                = 0 ;//add by yaoshulong 11.29.2017
...
public:
	...
	// add by yangyong 将Draw中的CBrush 替换成 HBRUSH
	virtual BOOL Draw2(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBkgnd = TRUE);
