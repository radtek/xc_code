...
class CSimpleTabCtrlItem
{
	friend class CSimpleTabCtrl;
private:
								CSimpleTabCtrlItem(CString sText, LPARAM lParam);
	void						ComputeRgn(BOOL fOnTop, BOOL bNewStyle = FALSE);
//	void						Draw(CDC& dc, CFont& font, BOOL fOnTop, BOOL fRTL, BOOL bNewStyle);
    void						Draw(CDC& dc, HFONT hFont, COLORREF clrBk,COLORREF clrExBk,COLORREF clrText, BOOL fOnTop, BOOL fRTL, BOOL bNewStyle);
	BOOL						HitTest(CPoint pt)			{ return (/*m_bShape &&*/ m_rgn.PtInRegion(pt)) ? TRUE : FALSE; }
	void						GetRegionPoints(const CRect& rc, CPoint* pts, BOOL fOnTop, BOOL bNewStyle) const;
    void						GetRegionPoints1(const CRect& rc, CPoint* pts, BOOL fOnTop, BOOL bNewStyle) const;
    void						GetRegionPoints2(const CRect& rc, CPoint* pts, BOOL fOnTop, BOOL bNewStyle) const;
	void						GetDrawPoints(const CRect& rc, CPoint* pts, BOOL fOnTop, BOOL bNewStyle) const;
	void						operator=(const CSimpleTabCtrlItem &other);
private:
	CString						m_sText;
	LPARAM						m_lParam;
	CRect						m_rect;
...
#define SimpleTabCtrl_CLASSNAME    _T("CSimpleTabCtrl")  // Window class name

class CSimpleTabCtrl : public CWnd
{
public:
    ...
    void                      SetSelItemBkColor(COLORREF clr);
    void                      SetSelItemExBkColor(COLORREF clr);
    void                      SetUnselItemBkColor(COLORREF clr);
...
private:
    ...
    COLORREF        m_clrSelItemBkColor;
    COLORREF        m_clrSelItemExBkColor;
    COLORREF        m_clrUnselItemBkColor;
	...
	CEdit						m_ctrlEdit;
	CArray <CSimpleTabCtrlItem*,CSimpleTabCtrlItem*>	m_aItems;
};
