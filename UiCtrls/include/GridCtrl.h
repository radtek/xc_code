...
#include "GridCellHotkeyEdit.h"
#include "GridCellNumberEdit.h"
#include "GridCellDoubleEdit.h"
#include "GridCellTime.h"
...
enum
{
    IDC_INPLACE_CONTROL_COMBO = 0x1001,
    IDC_INPLACE_CONTROL_CHECK,
};
enum BtnEditDataType
{
	TYPE_DOUBLE = 0,
	TYPE_INT
};
...
#define GVN_CELLMOUSEHOVER     (WM_USER + 1000)
...
///////////////////////////////////////////////////////////////////////////////////
public:
    void SetCellCheck(int nRow, int nCol, BOOL bChecked = FALSE);
    BOOL GetCellCheck(int nRow, int nCol);
    ...
    /////add by yaoshulong 2017.12.19
    HBRUSH GetGridHighlightHbrush() const         {return m_hHighlightBrush; }
    COLORREF GetGridHighlightColor() const         {return m_clrHighlight; }
...
public:
	...
	CGridCellBase* GetCell2(int nRow, int nCol) const;   // Get the actual cell!
    ...
    BOOL SetItemEditable(int nRow, int nCol, BOOL bEditable);//add by yaoshulong 11.29.2017
    BOOL GetItemEditable(int nRow, int nCol) const;//add by yaoshulong 11.29.2017

	void SetItemBtnEditSetting(int nRow, int nCol, double fTick, int nPrecision, BtnEditDataType dateType = TYPE_DOUBLE);

    BOOL IsItemEditing(int nRow, int nCol);
    
    BOOL SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass);
    BOOL SetCellCheckBox(int nRow, int nCol);
    BOOL SetCellComboBox(int nRow, int nCol, const CStringArray& options, CString strSel);
    BOOL SetCellHotkeyEdit(int nRow, int nCol);
    BOOL SetCellNumberEdit(int nRow, int nCol);
    BOOL SetCellDoubleEdit(int nRow, int nCol);
    BOOL SetCellTime(int nRow, int nCol);
	BOOL SetCellButtonEdit(int nRow, int nCol);
...
///////////////////////////////////////////////////////////////////////////////////
 public:
	...
	void MySetSelectedRangeEx(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
		BOOL bForceRepaint = FALSE, BOOL bSelectCells = TRUE);
...
protected:
    ...
    HBRUSH m_hHighlightBrush;
    COLORREF m_clrHighlight;
    COLORREF m_clrHighlightSetFocus;
    COLORREF m_clrHighlightKillFocus;
	...
	BOOL m_bEnableScrollSelectRow;
	BOOL m_bEnableSelectHighlight;
	BOOL m_bEnableSelectOnTopMost;
	BOOL m_bGoLButtonUp;
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
		...
		if (gvdi.item.bEdit)
		{
			switch (gvdi.item.nCellStyle)
			{
			case CellStyle_CheckBox:
			{
				static CGridCellCheck cell1;
				cell1.SetState(gvdi.item.nState);
				cell1.SetFormat(gvdi.item.nFormat);
				cell1.SetImage(gvdi.item.iImage);
				cell1.SetBackClr(gvdi.item.crBkClr);
				cell1.SetTextClr(gvdi.item.crFgClr);
				cell1.SetData(gvdi.item.lParam);
				cell1.SetFont(&(gvdi.item.lfFont));
				cell1.SetMargin(gvdi.item.nMargin);
				cell1.SetText(gvdi.item.strText);
				cell1.SetGrid((CGridCtrl*)this);
				cell1.SetEditable(gvdi.item.bEdit);
				cell1.SetCheck(gvdi.item.bCheck);
				return (CGridCellBase*)&cell1;
			}
			case CellStyle_ComboBox:
			{
				break;
			}
			case CellStyle_ButtonEdit:
			{
				static CGridCellButtonEdit cell2;
				cell2.SetState(gvdi.item.nState);
				cell2.SetFormat(gvdi.item.nFormat);
				cell2.SetImage(gvdi.item.iImage);
				cell2.SetBackClr(gvdi.item.crBkClr);
				cell2.SetTextClr(gvdi.item.crFgClr);
				cell2.SetData(gvdi.item.lParam);
				cell2.SetFont(&(gvdi.item.lfFont));
				cell2.SetMargin(gvdi.item.nMargin);
				cell2.SetText(gvdi.item.strText);
				cell2.SetGrid((CGridCtrl*)this);
				cell2.SetEditable(gvdi.item.bEdit);
				cell2.SetTick(gvdi.item.fTick);
				cell2.SetPrecision(gvdi.item.nPrecision);
				return (CGridCellBase*)&cell2;
			}
			default:
				break;
			}
		...
		else
		{
			...
			cell.SetEditable(gvdi.item.bEdit);
...
// add by yangyong 20170508
// 不需要关心真实的顺序 在SortItems中使用
inline CGridCellBase* CGridCtrl::GetCell2(int nRow, int nCol) const
{
    if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols) 
        return NULL;
