...
class CFastTradeView : public CBaseView
{
	DECLARE_DYNAMIC(CFastTradeView)
public:
	CFastTradeView(CDockListener &listener);
	virtual ~CFastTradeView();

	static CBaseView *CreateView(CDockListener &listener) { return new CFastTradeView(listener); }
...
private:
	...
	void CreateControls();
	void InitialData();
	bool InitFastTradeSettingNeeds();

public:
	bool ensureSecCodeMarket(bool bLock, const CString &SecCode, CString &MarketCode);
...
protected:
	...
	afx_msg void OnBtnFastTradeSetting(NMHDR *pNmhdr, LRESULT *pResult);
