...
void CTradeDlg::RedrawCustomPool(string s_Path, CGridCtrlEx* pCustomPool)
{
	...
	for (auto iter = M_CustomPool.begin(); iter != M_CustomPool.end(); iter++)
	{
		CString s_str;
		// 市场
		switch (TradeFun::find_enumMarketCode(string(iter->second.MarketCode)))
		{
		case enumSSE:
			s_str = "上海";
			break;
	...
	pCustomPool->SetSelectedRangeEx(-1, -1, -1, -1, TRUE);
...
bool CTradeDlg::File2MCustomPool(string s_Path, map<CString, CCustomInfo> &M_CustomPool)
{
	...
	else
	{
		...
		{
			ExceptionInterrupt;
			LOG(ERROR) << "prase fail:" << reader.getFormatedErrorMessages();
			return false;
		}
	}
	return true;
...
void CTradeDlg::DealClickedRemovePick(string s_Path)
{
	...
	int sum = CListCtrl_CustomPool.GetItemCount();
	for (int i = 0; i < sum; i++)
	{
		if (CListCtrl_CustomPool.GetCheck(i) == TRUE)
		{
			CString stock_code = CListCtrl_CustomPool.GetItemText(i, ColumnName2Index(CListCtrl_CustomPool,Name_SecCode));
...
void CTradeDlg::DealKillFocusInsertStock(const int ID_InsertStock, string s_Path)
{
	CCustomInfo obj_CCustomInfo;
	/*校验添加代码是否合法*/
	if (GainAlertInfo(ID_InsertStock, obj_CCustomInfo) == false)
	{
		return;
	}
	/*写入添加代码*/
...
void CTradeDlg::DealClickedRefreshAlert(const int CtrlId)
{
	int ret = ((CButton *)GetDlgItem(CtrlId))->GetCheck();
	switch (ret)
	{
	case BST_UNCHECKED:
		bRefreshAlert = false;
...
bool CTradeDlg::SubscribePosition()
{
	...
	if (true == SPtr_CData->GetMAccount(SPtr_CData->fund_account, obj_MAccount))
	{
		for (iter_MAccount = obj_MAccount.begin(); iter_MAccount != obj_MAccount.end(); iter_MAccount++)
		{
			...
			SendSubscribeMarket(false, MarketCode, IDMarket_SubscribePosition, &sSubscribe1, SubList1, Sum_Subscribe);
	...
	socket_struct_SubscribeDetail SubList[MaxSize], int SubSize)
{
	...
	int ret;
	switch (TradeFun::find_enumMarketCode(string(MarketCode)))
	{
	case enumSSE:
	{
	...
	case enumSZSE:
	{
		ret = pCApiFun->Api_Market_SEZE->Subscribe(RefID, pSubscribe, SubList, SubSize);
		break;
	}
	default:
		LOG(ERROR) << "abnormal MarketCode";
	...
	const string &s_Fun, PtrData &SPtr_CData)
{
	...
	if (Size == 1)//单个指令
	{
		...
		{
			if (TradeFun::SendOrder(pCApiFun, ApiType, s_str, SPtr_CData) < 0)
			{
				ExceptionInterrupt;
...
bool CTradeDlg::UnSubscribe_All(CString MarketCode)
{
	...
	case enumSSE:
		...
		break;
	case enumSZSE:
		pAPI_Unsubscribe = pCApiFun->Api_Market_SEZE;;
		break;
	default:
		ExceptionInterrupt;
		LOG(ERROR) << "abnormal MarketCode";
...
// 		bFirstRedraw = true;
// 	}
// 	_MSpecialAlert rose_MSpecialAlert;
// 	_MSpecialAlert fall_MSpecialAlert;
// 	_MSpecialAlert::reverse_iterator iter;
// 	vec_Alert.clear();
// 	int nIndexHighlighted = -1;
// 	if (SPtr_CData->GetMAlert_Special('1', rose_MSpecialAlert) == true)
// 	{
// 		for (iter = rose_MSpecialAlert.rbegin(); iter != rose_MSpecialAlert.rend(); iter++)
// 		{
...
// 						count++;
// #endif
// 						if (SPtr_CData->obj_CAlertConfig.StockTemplate == ID_CustomizePool)
// 						{
...
// 	}
// 
// 	if (SPtr_CData->GetMAlert_Special('0', fall_MSpecialAlert) == true)
// 	{
// 		for (iter = fall_MSpecialAlert.rbegin(); iter != fall_MSpecialAlert.rend(); iter++)
// 		{
// 			if (iter->second->SecCode == highlightedSecCode)
// 			{
// 				nIndexHighlighted = vec_Alert.size();
...
// 				{
// 					/*保留跳出批次里涨跌幅最小的股票*/
// 					if (SPtr_CData->VerifybJump(iter->second->SecCode) == true)/*自动滚动天生不支持涨、跌幅同时开*/
// 					{
// 						highlightedSecCode = iter->second->SecCode;
// 						bJump = true;
// 						JumpIndex = index_vec_Alert;
...
// 	if (true == bAutoRoll)
// 	{
// 		fillPreviousRedrawList(rose_MSpecialAlert, fall_MSpecialAlert);
// 	}
// 
// 	int nSelected = 0;
// 	if (true == bJump)
...
// 	}
// 	if (bAutoRoll || pAlertGrid->GetEnableSelectHighlight())
// 	{
// 		pAlertGrid->SetSelectedRangeEx(nSelected, 0, nSelected, nColCount - 1, TRUE);
// 	}
// 	
...
void CTradeDlg::OnRedrawAlert(CGridCtrlEx* pAlertGrid)
{
	...
	if (SPtr_CData->GetMAlert_Special(warnningID, MSpecialAlert) == true)
	{
...
LRESULT CTradeDlg::OnClearAlert(WPARAM, LPARAM)
{
	//CListCtrl_Alert.DeleteAllItems();
	//ASSERT(CListCtrl_Alert.GetItemCount() == 0);
	return 0;
}
...
{
	switch (TradeFun::find_enumSide_EntrustEach(s_str))
	{
	case enumBuy:
		pNMCD->clrText = RGB(255, 0, 0);//红色
		break;
...
{
	switch (TradeFun::find_enumPriceType(string(DefalutGear)))
	{
	case enum涨停价:
	{
		CSecurity obj_CSecurity;
...
double CTradeDlg::GainBalance_MaxEnable(bool bFinancingBuy)
{
	...
	if (bFinancingBuy == true)//融资买入
	{
		...
		if (SPtr_CData->GetVCreditAsset(SPtr_CData->fund_account, SPtr_CData->s_user.c_str(), VCreditAsset) == false)
		{
...
bool CTradeDlg::VerifyBEntrustAmount(bool bDismantling, const int Id_BigCanBuyTotal, const int Id_BEntrustAmount)
{
	/*委托数量大于100 0000或者可买数量非法*/
	UInt_64 EntrustAmount = 0;
	UInt_64 BigCanBuyTotal = 0;
	if (GainLegalValue(BigCanBuyTotal, Id_BigCanBuyTotal) == false)
	{
		MessageBox("可买股数异常", "提示", MB_OK);
		return false;
	}
	if (GainLegalValue(EntrustAmount, Id_BEntrustAmount) == false)
	{
...
bool CTradeDlg::VerifyEntrustAmount(bool bBuyModule, const int ID_EntrustAmount)
{
	CString s_str;
	GetDlgItem(ID_EntrustAmount)->GetWindowText(s_str);
	if ("" == s_str || "0" == s_str)
	{
...
bool CTradeDlg::SetEnable_balance(CSimpleStatic* pEnableBalance)
{
	CString s_str;
...
bool CTradeDlg::SetBigCanBuyTotal(bool bFinancingBuy, CSimpleEdit* pEntrustPrice, CSimpleStatic* pBigCanBuyTotal)
{
	...
	{
		BigCanBuyTotal = 0;
		LOG(INFO) << "BigCanBuyTotal is 0";//fix
	}
	else
	{
...
LRESULT CTradeDlg::OnDisConnect_Market(WPARAM wParam, LPARAM lParam)
{
	/*发生重连后，立刻清理内存和界面数据，防止数据断档导致的数据丢失*/
	SPtr_CData->ClearMAlert_ALL();
	//CListCtrl_Alert.SetItemCount(0);
	return 0;
}
...
void CTradeDlg::SetGlobalFont()
{
	...
	if (FontStandard == strStyle.Trim())
	{
		...
		s_hFontTabSel = SingleFontSet.GetDefaultFont(DEFAULT_FONT_14B);
	}
	else if (FontMedium == strStyle.Trim())
	{
		...
		s_hFontTabSel = SingleFontSet.GetDefaultFont(DEFAULT_FONT_16B);
	}
	else if (FontBig == strStyle.Trim())
	{
		s_hFontOrder = SingleFontSet.GetDefaultFont(DEFAULT_FONT_16B);
		s_hFontGrid = SingleFontSet.GetDefaultFont(DEFAULT_FONT_18N);
		s_hFontTab = SingleFontSet.GetDefaultFont(DEFAULT_FONT_18N);
		s_hFontTabSel = SingleFontSet.GetDefaultFont(DEFAULT_FONT_18B);
	}
}
